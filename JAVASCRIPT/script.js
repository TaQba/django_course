...
function stringTimes(str="Hi", n=1) {
    var output = '';
    for (i=0; i<n; i++) {
        output += str;
    }
...
function caught_speeding(speed, is_birthday){
    if (is_birthday) {
        return ticketRate(speed-5);
    } else {
        return ticketRate(speed);
    }
}
function ticketRate(speed) {
    if (speed > 80) {
        return 2;
    } else if (speed > 60) {
        return 1;
...
function makeBricks(small, big, goal){
  var smallSize = 1;
  var bigSize = 5;
  return(smallSize*small + bigSize*big) >= goal ? true : false
}
